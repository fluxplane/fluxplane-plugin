...
- {{code "fluxplane-plugin operation invoke <plugin> <operation> --input {...}"}} — call an operation; input is a JSON object. Add {{code "--dry-run"}} to validate input locally first, {{code "--result-only"}} or {{code "--field <dot.path>"}} to trim the output ({{code "*"}} maps over arrays, e.g. {{code "items.*.name"}}). Every failure is one JSON envelope ({{code "{plugin, operation, error: {code, message, fields, details}}"}}) — on stderr normally, on stdout under {{code "--result-only"}} so piped parsers read the error instead of crashing on empty input. Unknown fields and operations come back with a did-you-mean suggestion or the valid vocabulary. Large payloads exceed the OS argv limit — pass {{code "--input-file payload.json"}} or pipe with {{code "--input -"}} instead of inlining.
